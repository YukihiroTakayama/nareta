...
struct RewardListView: View {
    ...
    @State private var purchasingReward: Reward?
    @State private var deletingReward: Reward?
    ...
    var body: some View {
        ...
        NavigationStack {
            List {
                ...
                heroCard(available: available).cardRow(top: 8, bottom: 14)
                ...
                ForEach(active) { reward in
                    row(reward, available: available)
                }
            ...
            .sheet(item: $editingReward) { RewardEditorView(reward: $0) }
            .sheet(item: $purchasingReward) { RewardPurchaseView(reward: $0, available: available) }
            .sheet(isPresented: $showSettings) { SettingsView() }
            .rewardDeleteDialog($deletingReward) { RewardService(context: context).deleteReward($0) }
        }
    }
    ...
    private func heroCard(available: Int) -> some View {
        HeroCard {
    ...
    private func row(_ reward: Reward, available: Int) -> some View {
        ZStack {
            NavigationLink(value: reward) { EmptyView() }.opacity(0)
            RewardRow(reward: reward, available: available)
        }
        .cardRow()
        .modifier(RewardRowActions(
            canPurchase: !reward.isPurchased && available >= reward.price,
            onPurchase: { purchasingReward = reward },
